        ...
        for table_id in table_id_list:
            if table_id:
                ...
                if table_type == 'simpleTable':
                    ...
                    table_df.loc[1:, 'showQuestion'] = table_df.loc[1:, 'showQuestion'] + f'({table_question_id} != 1)'

                elif table_type == 'complexTable':
                    col_df = table_df[table_df.isTableColumn == '1'].reset_index(drop=True)
                    row_df = table_df[
                        (table_df.isTableColumn != '1') & (table_df.index != 0)].reset_index(
                        drop=True)
                    ...
                    cell_df[
                        'questionBody'] = cell_df.questionBody + '\n' + \
                                          col_df.questionBody.tolist() * len(row_df)
                    cell_df['questionNote'] = ''

                    # S_ questionId
                    cell_df['rowQuestionId'] = table_id + cell_df.questionId
                    cell_df['colQuestionId'] = cell_df.tableId + \
                                               col_df.questionId.tolist() * len(row_df)
                    cell_df['questionId'] = table_id + cell_df.questionId + \
                                            col_df.questionId.tolist() * len(row_df)
                    ...
                    cell_df['showQuestion'] = cell_df.apply(
                        lambda row: f'({row.rowQuestionId} != 1) & ({row.colQuestionId} != 1)',
                        axis=1)
                    cell_df.drop(columns=['rowQuestionId', 'colQuestionId'], inplace=True)
