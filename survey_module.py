...
class SurveyModule:
    ...
    def create(self, email):
        ...
        update_url = f'{main_url}?action=update&on=survey_module&gsid={gsid}'
        worksheet.update_value('A3', f'=HYPERLINK("{update_url}", "連結此問卷模組至專案")')

        # S_1-7 '刪除此問卷模組設定' 連結
        delete_url = f'{main_url}?action=delete&on=survey_module&gsid={gsid}'
        worksheet.update_value('A4', f'=HYPERLINK("{delete_url}", "取消連結此問卷模組至專案")')
    ...
    def update(self, gsid):
        try:
            ...
            survey_module_info = spreadsheet.worksheet_by_title('問卷模組資訊') \
                .get_values(start='C2', end='C6', include_all=True)

            survey_module_info_dict = {
                'surveyModuleId': gsid,
                'surveyModuleName': survey_module_info[0][0],
                'customSurveyModuleId': survey_module_info[1][0],
                'customProjectId': survey_module_info[2][0],
                'customTeamId': survey_module_info[3][0]
            }
            survey_module_worksheet_name = survey_module_info[4][0]
            ...
            if not survey_module_worksheet_name:
                return '問卷模組資訊不能為空!'
